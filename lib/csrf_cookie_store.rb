...
module ActionDispatch
  module Session
    ...
    class CsrfCookieStore < Rack::Session::Abstract::ID
      ...
      def destroy_session(env, fakktion_session_id, options)
        new_sid = generate_sid unless options[:drop]
        # Reset hash and Assign the new session id
        env["action_dispatch.request.unsigned_session_cookie"] = new_sid ? { "fakktion_session_id" => new_sid } : {}
        new_sid
      end
      ...
      def load_session(env)
        stale_session_check! do
          ...
          data = persistent_fakktion_session_id!(data)
          [data["fakktion_session_id"], data]
        end
      end

      private

      def extract_fakktion_session_id(env)
        stale_session_check! do
          unpacked_cookie_data(env)["fakktion_session_id"]
        end
      end
      ...
      def persistent_fakktion_session_id!(data, sid=nil)
        data ||= {}
        data["fakktion_session_id"] ||= sid || generate_sid
        data
      end

      def set_session(env, sid, session_data, options)
        session_data["fakktion_session_id"] = sid
        session_data
      end

      def set_cookie(env, fakktion_session_id, cookie)
