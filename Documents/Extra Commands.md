...
4. Checkout ```sudo nano /etc/default/locale``` and confirm the ENV VARS created (Make sure that there are no spaces between constant and value.) 
5. Navigate to Documents folder, and execute ```./install_reqs.sh```, then precompile Fakktion with ```rake assets:precompile```.
6. PUMA SETUP: 
